 ...
 * <p>
 * See https://developer.android.com/reference/android/widget/OverScroller
 */
public class OverScroller {
    private static final Logger log = LoggerFactory.getLogger(OverScroller.class);
    ...
    /**
     * Creates an OverScroller with a viscous fluid scroll interpolation and flywheel.
     */
    public OverScroller() {
        this(null);
    }
     ...
     *
     * @param interpolation The scroll interpolation. If null, a default (viscous) interpolation will
     *                      be used.
     */
    public OverScroller(Interpolation interpolation) {
     ...
     * @hide
     */
    public OverScroller(Interpolation interpolation, boolean flywheel) {
        if (interpolation == null) {
            mInterpolation = Interpolation.exp5;
        } else {
     ...
     * is {@link OverScroller#SCROLL_FRICTION}.
     *
    ...
    /**
     * Method added in VTM:
     * Match animation durations of x and y scroller to appear smoother.
     */
    public void matchDurations() {
        if (mScrollerX.mDuration < mScrollerY.mDuration) {
            mScrollerX.mDuration = mScrollerY.mDuration;
        } else {
            mScrollerY.mDuration = mScrollerX.mDuration;
        }

        if (mScrollerX.mSplineDuration < mScrollerY.mSplineDuration) {
            mScrollerX.mSplineDuration = mScrollerY.mSplineDuration;
        } else {
            mScrollerY.mSplineDuration = mScrollerX.mSplineDuration;
        }
    }
        ...
        switch (mMode) {
            case SCROLL_MODE:
                ...
                if (elapsedTime < duration) {
                    final float q = mInterpolation.apply(elapsedTime / (float) duration);
    ...
    public int timePassed() {
        final long time = currentAnimationTimeMillis();
    ...
    static class SplineOverScroller {
        ...
        SplineOverScroller() {
            mFinished = true;
            final float ppi = CanvasAdapter.dpi;
            mPhysicalCoeff = GRAVITY_EARTH // g (m/s^2)
                    * 39.37f // inch/meter
                    * ppi
        ...
        private void startAfterEdge(int start, int min, int max, int velocity) {
            if (start > min && start < max) {
                log.error("OverScroller", "startAfterEdge called from a valid position");
                mFinished = true;
                return;
            }
