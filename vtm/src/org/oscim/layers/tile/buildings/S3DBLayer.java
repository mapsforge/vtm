...
/**
 * A experimental Layer to display S3DB Elements
 */
public class S3DBLayer extends BuildingLayer {

    private final float TILE_SCALE = (S3DBUtils.REF_TILE_SIZE / (Tile.SIZE * COORD_SCALE));
    ...
    public void processElement(MapElement element, ExtrusionStyle extrusion, MapTile tile) {
        ...
        S3DBUtils.mapPolyCoordScale(element);
        float minHeightS = S3DBUtils.mapGroundScale(minHeight, groundScale) * TILE_SCALE;
        float maxHeightS = S3DBUtils.mapGroundScale(maxHeight, groundScale) * TILE_SCALE;
        float minRoofHeightS = S3DBUtils.mapGroundScale(maxHeight - roofHeight, groundScale) * TILE_SCALE;
